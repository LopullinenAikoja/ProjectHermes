...
for p = 1:CTRL_PARS.NTest
    if (CTRL_PARS.ForwardMode == 0)
        if (CTRL_PARS.Sample_Type(CTRL_PARS.TestPtr(p))>2)
            ...
            ntotal(pk(1)) = length(PROFILER_DATA.DataMats{CTRL_PARS.TestPtr(p)});
            r_sample(pk(1)) = ceil(ntotal(pk(1))*rand(nrecords(pk(1),1)));        
        elseif (CTRL_PARS.Sample_Type(CTRL_PARS.TestPtr(p))==2)
            ...
            ntotal(pk(1)) = length(PROFILER_DATA.DataMats{CTRL_PARS.TestPtr(p)});
            r_sample = ceil(ntotal(pk(1))*rand(nrecords(pk1),1)));
